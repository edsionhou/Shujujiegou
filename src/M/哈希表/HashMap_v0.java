...
public class HashMap_v0<K, V> implements Map<K, V> {
    ...
    private static final int DEFAULT_CAPACITY = 1 << 4;   // 2^4  16
...
//            com = compare(key, node.key, keyCode, node.hash);
            K k2 = node.key;
            int k2hashCode = node.hash;
            if (keyhashCode > k2hashCode) {  //k2是树上的点
//                node = node.right;
                com = 1;
...
//                node = node.left;
                ...
                int c = ((Comparable) key).compareTo(k2);  //不能采用Comparable  com=0的情况
                if(c!=0){
                    com =c ;
                }
                ...
                if ((node.left != null && (result = getNode(node.left, key)) != null)
                    ...
                    //找不到   随便选个地方加把
                    //细想：能来到else，必定扫描了 Node的所有子节点，如果进入下面的if，又要来一次while循环，
                    //又要递归扫描，没必要，所以加个标记。
                    searchKey = true;
                    com = System.identityHashCode(key) - System.identityHashCode(k2);
    ...
    private Node<K, V> getNode(Node<K, V> node, K k1) {   //扫描方法 递归遍历节点左右子树是否有相同的key
        ...
        Node<K, V> result = null;
        while (node != null) {
            /*int compare = compare(key, root.key,h1HashCode , root.hash);
                                    //Node 建立了hash 非常关键，否则每次都计算KEY的hashCode值
            ...
            int k2hashCode = node.hash;
            K k2 = node.key;
            //先比较哈希值   h1-h2不好用，因为如果溢出了，正负你无法估计
            if (k1HashCode > k2hashCode) {
                ...
                return node;
                    ...
                    && k1 instanceof Comparable) {  // 看看能否用Comparable比较
...
//                    return node;
                    //不能算相等， 我们要以equals作为标准， Comparable不行，如果compare用的标准和equals不同，
                    //我们就错了。
                }

            }//哈希值相等，不具备比较性,equals也不等  先不考虑不同类  考虑null
    ...
    public void print() {
        ...
        for (int i = 0; i < 1; i++) {
            Node<K, V> root = table[i];
                System.out.println("index等于 "+table[i]);
